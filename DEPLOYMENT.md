...
- [x] **Customer authentication** — shipped iter 19 (signed-intent path). Customer signs an EIP-712 `DepositIntent` off-chain (banking-app key, hardware wallet, EIP-1271 contract sig). Backend verifies the signature recovers `intent.customer`, checks `expiry`, rejects reused `nonce`. Toggle `PAYCODEX_REQUIRE_INTENT=true` to make signed intents mandatory on every deploy. Helper CLI: `node scripts/sign-intent.mjs`. Production: combine with KYC + tax residency cert flow upstream of the signing UI.
